...
public class SimplePlanScene : MonoBehaviour
{
    ...
    void Start()
    {
        if (generateOnStart)
        {
            Debug.Log("🚀 Creating Simple Plan Scene...");
            CreatePlanScene();
        }
    }
    ...
    void ClearScene()
    {
        ...
        foreach (GameObject obj in existingObjects)
        {
            if (obj.name.StartsWith("Plan_") || obj.name.StartsWith("Technician_") || 
                obj.name.StartsWith("Environment_") || obj.name == "Canvas")
            {
    ...
    void CreateSphereTechnician(string name, string role, Vector3 position, Color color)
    {
        ...
        SphereCollider collider = technician.GetComponent<SphereCollider>();
        // Collider already exists from CreatePrimitive
    ...
    void CreateSimpleUI()
    {
        ...
        // Add CanvasScaler and GraphicRaycaster with proper error handling
        try
        {
        ...
        {
            Debug.LogWarning($"Could not add UI components: {e.Message}");
        }
        ...
        infoText.text = "Warehouse Plan Scene - Click on sphere technicians to select • WASD to move • R to return";
